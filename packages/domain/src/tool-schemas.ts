...
function buildSchemaRegistry(): Record<string, SchemaEntry> {
  ...
  // Intentional Divergence (Traderton): platform tool schema entries
  // `publish_artifact.location`, `publish_artifact.metadata`, and
  // `execute_code.dependencies` are omitted — those tools are platform, not
  // trading (see docs/003 Phase-9a). Trading schema entries are retained verbatim.
