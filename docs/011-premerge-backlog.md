...
- [~] **B3. Watch tools → boundary (ATOMIC slice — Option D, 004 decision-log).** herobids `tools/watch.ts`. Corrected scope (`resolve_watch` is NOT here — it lives in `resolvers.ts`, routes with the resolvers group later). Decision agent ruled Option D (settled within rules, no ratification): Traderton `check_watches` = evaluation authority; the herobids platform monitor becomes a thin boundary consumer. Must land ATOMICALLY:
  ...
  - [ ] **B3-tools** (herobids `tools/watch.ts`): re-point `list_watches` (read path, in-process fallback) + `watch_token`/`remove_watch`/`check_watches` (write path, fail-closed) over the boundary. Mirror risk-limits.ts/analytics.ts B2 patterns. Route all 4 atomically (no split-brain).
  - [ ] **B3-monitor** (herobids `market-intelligence/monitor.ts`): `evaluateWatches` stops scanning/evaluating local `agent:watches:*`; instead invokes boundary `check_watches` per active+subscribed agent (per-agent subject write boundary + resolve ownerId per agent) and consumes `triggered[]`; keeps platform work local (session/prefs/scanner-gated filters, dedupe, rate-limit, `emitMarketWatchTriggered`/`enqueueWake`).
  - [ ] **B3-summary** (herobids `agent.ts` `loadActiveWatchSummary`): source `agent:watches:summary:*` over the boundary (else the agent's own tick view of its watches goes stale — 2nd split-brain).
  - Fallback ONLY if monitor+summary prove larger than one slice: Option C (land B3-tools with local watch-wake path DISABLED + logged Deferred-required row). Never Option A (silent degrade).
