...
| market-assessment platform orchestration + platform billing | Left behind (Intentional divergence + Deferred) | Only the domain/analysis is trading-owned; platform-side billing stays outside Traderton, while Traderton-owned usage metering remains Deferred (decision 15). **Table-level clarification (c4.9h ruling, 2026-09-15):** the `market_assessment_requests`/`_runs`/`_artifacts` **tables are herobids-local PLATFORM tables** (agent preset-review/strategy-selection runtime — decisions 7–9; same class as the wake-gate / `PresetTransitionPort` / `assess_strategy_preset`/`change_strategy_preset` KEEP rows). Traderton carries `runs`/`artifacts` **schema/types ONLY** (copied domain slice; VERIFIED zero runtime access — no repo/tool/read/write) for the DEFERRED agent-reasoning-runtime relocation; `requests` was never copied. These tables are **NOT in the trading-drop set (c4.9f)**; the ~20 herobids-worker access sites are legitimate platform-runtime access. Re-point/Gap-drop both REJECTED (would author a runtime Traderton doesn't own / silently degrade live platform behaviour). |
