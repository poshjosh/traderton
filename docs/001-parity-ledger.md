...
- **g3 — instrument population (InstrumentRepository.upsertInstruments write): DROP — Deferred (required for cutover).** herobids-worker `instrument-population.ts` removed. Traderton owns `instruments` + a byte-identical `populateInstrumentsFromVenues` but it is NOT yet wired into `create-trading-runtime` startup. Traderton MUST wire its copied population at worker startup before cutover, else `find_instrument` reads empty. No data migration (greenfield). Successor: Traderton runtime startup.
