...
**Next action:** Phase 8 (`apps/worker` — trading loop, LARGE SUBTRACTION), per the roadmap [009-extraction-roadmap.md](./009-extraction-roadmap.md). Mixed trading+platform (~173 files, ~45 `agent-*`); requires formal written keep/delete classification before code moves; expect ownership questions and fused seams. Seed plan at [docs/features/008-worker-plan.md](./features/008-worker-plan.md). Phases 8–10 governed by 009; execute each via its per-phase pattern, honoring the stop-gates.
