...
**Next action:** Phase 7 (`backtesting`), per the roadmap [009-extraction-roadmap.md](./009-extraction-roadmap.md). Clean-package: copy whole `packages/backtesting` (replay, historical execution, replay corpora). Depends on domain + engine. Seed plan at `docs/features/007-backtesting-plan.md`. Phases 7–10 governed by 009; execute each via its per-phase pattern, honoring the stop-gates.
