...
- Config seam progress: resolved the `WakePreferences`/`agent-protocol` seam by clean in-place deletion (deleted platform wake schema, `agent-protocol.ts`, `config/load-providers.ts`, `config/assessment-config.ts`; trimmed barrels). **Build + copied tests green at this checkpoint (512 tests).**
- **BLOCKED on two herobids source-change requests** (per the source-fix rule; logged in [003-anomalies-and-deviations.md](./003-anomalies-and-deviations.md)):
  - **#1 strategy registry** — `initStrategyRegistry` hard-requires the LLM/hybrid schemas, fusing them into the trading strategy registry. Need herobids to make llm/hybrid optional (behaviour-preserving) so Traderton can copy a mechanical-only registry.
  - **#2 blueprint union** — the blueprint revision payload fuses agent (platform) and bot (trading) payloads in one discriminated union the whole file depends on. Need herobids to separate the bot blueprint slice (behaviour-preserving) so Traderton copies an already-split bot payload.
  The remaining bulk platform-schema deletions in `config/schema.ts` are clean and queued behind these two, because the agent-runtime-policy schemas are consumed by the blueprint agent-payload (#2) and the LLM schemas by the strategy registry (#1).
