...
| **A′. S-1 mechanical-only narrowing** | **AUTHORED (1 line + test)** | Deliberate divergence (decision 3 / decision 2). Tiny: narrow `decisionMode` to `['mechanical']` in a Traderton wrapper + update the copied acceptance test + a live assertion. Human-approved (option a). |
