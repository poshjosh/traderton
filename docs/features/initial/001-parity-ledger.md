...
| market-assessment platform orchestration + platform billing | **Temporary platform retention — reassessment required before legal-isolation sign-off** | **Current behavior is unchanged:** `market_assessment_requests`/`_runs`/`_artifacts` remain in Herobids and are excluded from the completed c4.9f trading-table drop; the worker has ~20 live read/write sites. Traderton carries `runs`/`artifacts` schema/types only (zero runtime consumer; `requests` was never copied). **This supersedes c4.9h's final-sounding table classification:** temporary retention preserves the working agent-preset review, platform billing, agent policy, wakes, and binding lifecycle while the broader legal-isolation question is deliberately decided. It is not a final finding that the capability lies outside Traderton's boundary. The capability consumes Traderton market/scoring evidence and selects a preset used for future trading entries; it therefore remains trading-adjacent. Before legal-isolation sign-off, route the whole capability through 008: request/billing/idempotency, evidence/scoring, platform LLM ranking, cached artifacts, review wakes, and preset bindings/transitions. Options are (1) permanent Herobids retention as an explicit exception, (2) complete Traderton ownership, (3) a deliberate boundary split, or (4) removal/deprecation. See 004, 011, CANONICAL-STATE, and Slice 4. |
...
| All `Deferred (required for cutover)` entries resolved | **Met (engineering) — 2026-09-15** | Reconciled (see "Cutover-gate reconciliation" at the end of this doc): checkVenueAccountLimit (count_venue_accounts), adjust_risk_limits (write+read re-point), swap `score_candidate` (T2 Option B), watch/price/market-data agent read tools (re-pointed; registry not constructed; only type-only imports remain), economic-calendar acquisition (moved to the boundary — earlier "UNMOVED" note was stale), and the herobids bot-lifecycle functional suite (now boundary-aware, no it.skip) are all `Met`/`Improved`. Metering is `Deferred (optional)` (no parity obligation). Accepted `Gap`s pending explicit human sign-off at cutover: g1 decision-failure + g2 llm-artifact persistence (greenfield → no history lost). No capability leaves herobids-on-Traderton weaker than herobids-today. |
| Side-effecting parity validated | Pending | Copied tests and A8/C5 evidence exist, but the Step 16 REST differential against pinned pre-removal Herobids oracle `1f6978d740d45e466cf4149617b8afc1c721e751` is still outstanding. |
...
| Operational readiness passed | Pending | Step 16 must still produce the 007 equivalence and representative-load reports plus latency, restart-resilience, and rollback evidence in the target environment. A8/C5 and soak evidence are supporting, not substitutes. |
...
are routed over REST. Plan: [docs/features/initial/features/L3-Rx-subject-resolver-venue-signal-plan.md](./features/L3-Rx-subject-resolver-venue-signal-plan.md).

  **Plan (implemented):** [docs/features/initial/features/L3-Rx-subject-resolver-venue-signal-plan.md](./features/L3-Rx-subject-resolver-venue-signal-plan.md)
...
The 008 decision agent settled the bot-consumer contract (see 004 2026-09-12 + docs/features/initial/09 brief). Consequences for the ledger:
...
### c4.9d-FG — blueprint→bot instantiate write over the boundary (`instantiate_bot`) — IN PROGRESS (2026-09-12)
The last remaining trading-table WRITE in herobids (`blueprints.ts` POST /instantiate bot branch `tx.insert(bots)` + its venueAccounts binding read) is being re-pointed over the boundary. Human APPROVED authoring; decision-agent design ruling SETTLED (see 011 c4.9d + 003 atomicity-split entry). NEW traderton sibling tool `instantiate_bot` (create_bot untouched): copied drive-target create path WITHOUT the mark-running + start-enqueue steps (stopped-create), caller-supplied actorId, creatorType='user', persists blueprintId/revisionId/configSnapshot, keyed to the caller-supplied venueAccountId (connectionId dropped per decision 13, resolved locally). Idempotency via the EXISTING dispatchSideEffecting wrap (threaded idempotencyKey). Parity REACHED via boundary idempotent-create; the source's single-tx atomicity SPLIT is a bounded Intentional-divergence (residual = inert stopped-orphan on boundary-success/local-fail/no-retry; converges on retry). This is the sole remaining c4.9f drop blocker; once landed + a zero-access re-audit passes, c4.9f (the drop) proceeds.
### c4.9g — herobids-worker trading-repository usage removal (final c4.9f drop-blocker) — decision-agent per-usage rulings (2026-09-12)
The herobids WORKER (`apps/worker/src`) accessed trading tables INDIRECTLY via 4 repo classes. All are persistence/read side-channels, NOT trading logic (execution already crosses the boundary; the circuit-breaker is in-memory). Per-usage rulings:
- **g4 — `find_instrument` (InstrumentRepository.search read): RE-POINT (parity preserved).** The herobids agent-runtime `find_instrument` tool re-pointed OFF the local `instrumentRepo` ONTO Traderton's `find_instrument`, which is ALREADY exposed over the boundary (boundary/src/registry.ts includes instrumentTools; bin.ts wires instrumentRepo into the boundary context). Clean re-point; the successor tool already exists at parity.
- **g3 — instrument population (InstrumentRepository.upsertInstruments write): DROP from herobids + WIRED in Traderton — DONE (2026-09-15, was Deferred-required-for-cutover).** herobids-worker `instrument-population.ts` removed (c4.9g). Traderton now calls its byte-identical `populateInstrumentsFromVenues` in `create-trading-runtime.ts start()` (once, non-blocking, after the symbol-cache warmup), reusing the credential-less Hyperliquid + Bybit adapters already built there for the symbol cache (fetchMarketMetadata = public loadMarkets, no credentials). Decision-agent ruling: wire in the RUNTIME start() (adapters already exist there), NOT the boundary bin.ts (would duplicate adapter construction); copy-faithful (verbatim herobids index.ts:1745-1758 pattern). Verified: worker tsc + db/worker builds clean; create-trading-runtime + venue-instrument-cache suites 37/0. Cutover prerequisite CLEARED — find_instrument now reads a populated table. No data migration (greenfield).
- **g1 — decision-failure recording (DecisionFailureRepository.insert write): DROP — Gap (matches Traderton's own M1 no-op deferral; SIGN-OFF).** herobids-worker `AgentDecisionHandler.recordFailure` removed. Traderton's decision router (decision-intake.ts) already dropped `decisionFailureRepo.recordFailure` as an M1 no-op; the `insert` method + `get_agent_decision_failures` read exist but nothing writes in the Traderton decision path. Net: no side persists decision failures; the read returns empty until Traderton wires the write. Authoring a boundary write would re-introduce what the successor deferred → not faithful. No data migration (greenfield). Successor: Traderton decision path (write deferred).
- **g5 — reconciliation orphaned-position auto-close (PositionRepository.upsert write): DROP — Intentional divergence (already owned by successor).** herobids-worker `reconciliation-orphaned-cleanup.ts` removed. It had NO caller (dead code); Traderton's copied `AgentTradingActor` startup reconciliation already calls the byte-identical `cleanupOrphanedPositions` behind the boundary. No behaviour change; no data migration (greenfield). Successor: Traderton `AgentTradingActor` reconciliation.
- **g2 — LLM decision-artifact audit (LlmArtifactRepository.insert write): DROP — Gap (escalated → human confirmed Option 1, 2026-09-12).** herobids-worker `agent.ts` `onArtifact` write to `llm_decision_artifacts` removed. The writer is agent-reasoning-runtime code (runs in herobids, Intentional Divergence — never copied to Traderton); the table + repo are copied into Traderton's trading schema but have no writer. No side persists LLM artifacts until the agent reasoning runtime moves to Traderton (a later, larger slice). Option 2 (relocate to a herobids-platform table) REJECTED: would author new herobids-platform persistence + pre-commit the runtime-ownership answer while that question is open. No data migration (greenfield). Revisit when the runtime moves.
### c4.9i — herobids-worker BotRepository + PgJournal re-point/removal (final drop-blocker after the glob-tool false-green) — decision-agent rulings (2026-09-15)
Drop-time ripgrep re-audit found the worker still read 5 trading tables via `BotRepository` + `PgJournal` (hidden by an earlier glob-tool false-green). Bot lifecycle WRITES already route over the boundary (sideEffectBoundary.invokeAndAwait). Per-usage rulings:
- **Bot/position/analytics READS (getBotsByCreator/getBotById/getOpenPositionsByCreator/getAnalyticsByCreator):** the agent TOOLS (bots/analytics/account) are already boundary-first → **A' remove the dead local botRepo fallback**. The broker `BOT_QUERY` query path (777/805/849/887) is **DEAD** (ripgrep: no publisher of `agent.bot.query`) → remove. Genuine **A re-points** (live, unconditional local reads): resolvers.ts:31 (`resolve_bot` → boundary `list_bots` + in-app filter), risk-limits.ts:154/155 (→ `list_positions` + `get_analytics.realizedPnlUsd`), and TWO in-RUNTIME reads agent.ts:2696/3099 (tick-gate + hard-limit-stop → `list_positions` via the module `tradertonReadBoundary`). Successors all EXIST (list_bots/get_bot_status/list_positions/get_account_summary/get_analytics).
- **Bot state WRITES (markBotStopped/markBotRunning/restoreBotRuntimeState/updateBotConfig):** **A' remove-dead-mirror** — zero consumers (lifecycle already boundary-owned; c4.5 delete-mirror class). The whole agent.ts `toolBotRepo` facade + `ctx.botRepo` collapses.
- **getResolvedVenueAccount (index.ts:492):** reads `connections` (KEEP, NOT venue_accounts) → **A' remove-redundant** (use the capability-descriptor's `resolvedVenueAccountId` already in hand). Approval-path → human confirm (minor).
- **PgJournal.append (agent-activity → journal_events, agent.ts:1788/broker:1035):** **C Gap-drop** — Traderton OWNS journal_events (writes the trade-event truth; get_agent_journal_events reads it); the herobids agent-activity write is a legacy parallel write with no surviving dedicated reader (preset-transitions read the KEEP agent_preset_transitions table). NOT a K re-classify (would fork a Traderton-owned trading table). Agent-activity persistence deferred until the agent-reasoning-runtime relocates.
- **⚠ AlertDispatcher journal_events POLL (alert-dispatcher.ts:126/268/300 scanAfter/getByIds/getById) — HUMAN ESCALATION / separate drop-blocker.** A herobids PLATFORM subsystem continuously polls journal_events for Telegram trade-alert delivery; its severity map keys are exclusively Traderton-owned trade-event types (risk.*/execution.failure/instance.*/reconciliation.*). No STREAMING/tail boundary read exists (get_agent_journal_events is a synchronous agent-scoped snapshot, not a cross-owner tail). Options: (a) author a new Traderton alert-feed/streaming boundary (four-risk); (b) Gap-drop → lose Telegram trade alerts; (c) block the drop until authored. Same weight-class as the c4.9c decisions-family flag. journal_events (and thus c4.9f) stays BLOCKED on this regardless of the rest of c4.9i.
### c4.9j — AlertDispatcher trade-event alerting = Met (2026-09-15). Deferred-required-for-cutover parity obligation preserved over the boundary as a polled cross-owner read.
The feed's SHAPE question (streaming/tail boundary vs cross-owner journal read) was SETTLED as a **polled cross-owner read** (the dispatcher already polls under its own Redis-lease/at-least-once loop; no streaming boundary needed). Implemented:
- **traderton — 3 new `read-database` boundary tools** (`packages/worker/src/tools/bots.ts`, registered in `botManagementTools`): `scan_trade_events` (`{cursor?:{createdAt ISO,seenIds},typePrefixes?,limit}` → `{ok,events}`; ISO→Date cursor parse; `PgJournal.scanAfter` reproduced VERBATIM), `get_events_by_ids` (`{ids}` → `{ok,events}`), `get_event_by_id` (`{id}` → `{ok,event|null}`). NO owner/actor filter — deliberately cross-owner (`scanAfter` is already cross-owner; the consumer's `typePrefixes` is the only filter). System-subject-scoped by convention; fenced by the consumer's `allowedActorTypes:['system']` grant (no per-tool allow-list exists in the boundary config surface — OQ-2 accepted actor-type-only fencing, matching the market-intel system-read precedent). Added to 005; capability status below.
- **herobids — thin re-point** (KEEP the dispatch loop / Redis lease / cursor / cooldown / delivery / `evaluateAlertPolicy` untouched): introduced a narrow `TradeEventFeed` port; swapped `AlertDispatcher`'s constructor `PgJournal`→`TradeEventFeed` (mechanical body change); authored a boundary-backed adapter (mirrors `createTradertonReadBoundary`; Date→ISO outbound cursor + ISO→Date inbound `createdAt` rehydration mirroring `evidence-row-mappers` `toJournalRow`; THROWS on failure/transport_error/in_progress so the tick reschedules WITHOUT falsely advancing the cursor); wired a system-subject read boundary (`actor {type:'system',id:'alert-dispatcher'}`, distinct from `market-intel`); OQ-4 degrade = symmetric config-guard (dispatcher not started when the feed boundary is unconfigured, mirroring the existing Telegram warn-and-return); removed `const journal = new PgJournal(db)` + the now-unused `PgJournal` import.
- **This removes the LAST herobids `journal_events` reader → UNBLOCKS the `journal_events` drop at c4.9f.** Ripgrep confirms ZERO local `PgJournal`/`journal_events` readers remain in herobids `apps` (only the boundary tool name `get_agent_journal_events`, comments, and the new port doc).
- **Verified:** traderton worker tsc + db/worker/boundary builds clean, `bots.test.ts` 148/0 (+13 for the 3 tools); herobids worker tsc clean, alert-dispatcher + adapter suites 23/0.
- **Parity:** Preserved (re-point). Intentional cross-owner exposure (actor-type-fenced) noted per R3.

**[historical framing, RESOLVED]** The feed was earlier framed as a Deferred (required-for-cutover) obligation with an open A/B/C shape choice. Gap-drop (lose Telegram trade alerts) was ruled out; the polled-read shape (above) met the obligation. The herobids AlertDispatcher consumes journal_events as a CROSS-OWNER, cursor-based forward TAIL (`scanAfter({cursor:{createdAt,seenIds},typePrefixes,limit})` ordered `asc(createdAt,id)` + `getById`/`getByIds` for retry) under a singleton Redis lease with at-least-once semantics — served now by the 3 cross-owner read tools.

### Cutover-gate reconciliation (2026-09-15, coordinator) — all Deferred-required entries verified RESOLVED

A final merge-readiness pass initially flagged 6 "open" Deferred-required entries, but grounding each against the ACTUAL code (ripgrep) showed the flag was reading stale PROBLEM-STATEMENT lines, not their later RESOLUTION lines. Verified current state:

- **checkVenueAccountLimit — Met.** herobids `apps/api/src/plan-guards.ts:62` sources the count from the boundary `count_venue_accounts` read tool (option C); policy stays herobids-owned. (Was 001 ~254.)
- **adjust_risk_limits — Met.** Write re-pointed over `tradertonWriteBoundary` (fail-closed); read (`get_risk_limits`) re-pointed T1. (Was 001 ~307.)
- **price/market-data swap/dex staleness (`score_candidate`) — Met.** T2 Option B built (GeckoTerminal token→pool), ratified. (Was 001 ~386.)
- **watch/price/market-data agent-container read tools — Met.** Re-pointed over the boundary; `createProviderRegistry`/`createPriceService` are no longer constructed in the herobids agent worker (only a comment at index.ts:123). The ONLY remaining `@herobids/market-data` references in the worker (`agent.ts`, `traderton/hybrid-price-adapter.ts`) are `import type` ONLY (accepted type-only exception; hybrid price data flows over the boundary via `createBoundaryPriceService`). The subject-resolver `ownerScopedNoVenue` signal (L3-Rx) already handles their scoping. (Was 001 ~457 + the "NEW read-tools obligation".)
- **economic-calendar acquisition — Met (this ledger's earlier "Deferred/UNMOVED" entry at ~493 is STALE, written mid-flight before the build landed).** Traderton `packages/boundary/src/bin.ts` runs the `CompositeEconomicCalendarProvider` acquisition loop (startup warm + `setInterval` refresh) and injects the provider into the boundary context; the `get_economic_calendar` tool (`packages/worker/src/tools/market-data.ts:506`) reads the cache `cacheOnly:true`; herobids `agent.ts:3373` reads it over the boundary (`parseEconomicCalendarBoundaryPayload`). No economic-calendar acquisition runs in the herobids process. RESOLVED.
- **pull-based usage metering — Deferred (OPTIONAL), NOT a cutover blocker.** 001 cross-cutting rows (~88/218) classify Traderton-native metering/payments/caps as Deferred (optional) with NO herobids parity obligation (platform keeps its own billing). Not gate-blocking.
- **herobids functional/E2E suites boundary-aware — Met.** The connection suites were made boundary-aware (`dbf0c4f1`); the previously-`it.skip` bots-lifecycle cases are now un-skipped and boundary-aware (create_bot async-over-boundary returns {botId}; DELETE/stop/start resolve existence/ownership/status over the stubbed boundary via `get_owner_bot_status`/`delete_bot`). Ripgrep confirms ZERO `it.skip`/`describe.skip` in `apps/api/src/__tests__/functional/` (only the standard DATABASE_URL `skipIf`). The c4.9f local-`bots`-table drop forced+settled the "no local mirror" contract. api tsc clean; suite loads clean.

**Accepted Gaps needing explicit human sign-off at cutover (NOT blockers, greenfield → no history lost):** g1 decision-failure persistence + g2 llm-artifact persistence (neither side persists until the agent-reasoning-runtime relocates).

**NET: every `Deferred (required for cutover)` entry is Met/Improved (or a signed-off-pending Gap). The engineering path to cutover is COMPLETE.** What remains for the merge gate is inherently HUMAN/operational: (2) all tests pass in a full cross-stack run, (3) run locally + on staging for a while (manual/visual/black-box soak), (4) explicit human merge approval — plus the operator-config touchpoints (the alert-dispatcher consumer `allowedActorTypes:['system']`; boundary `SCRAPFLY_API_KEY`/`LLM_*` env; matching HMAC signing creds). None of these are code deliverables — they are the human's cutover run.
