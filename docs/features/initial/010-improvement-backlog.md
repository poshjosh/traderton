...
| B11 | **Migrate `validate-swap-venue.sh` (herobids) to traderton** | Operator-only bash probe (no CI role): hits the herobids API `/health` + direct venue-quote HTTP probes; imports no adapter code, so it is **not a parity test** — it validates venue reachability only. Deferred from the Slice 2 validator migration (`docs/features/initial/features/slice-2-migrate-venue-validators.md` §2) to avoid scope creep; its value is low (venue reachability is better covered by the migrated launch validators' quote steps + the venue integration suites). | If ever wanted: copy it into `traderton/scripts/shell/tests/`, retarget its API probe from the herobids API to traderton (or drop that leg), and re-point its direct venue HTTP probes; otherwise leave it as a herobids-only operator tool. | Low | Low | Low |
| B12 | **Fix the launch validators' confirmation-status field (source in herobids, then re-copy)** | The migrated `validate-{jupiter,1inch}-launch.ts` confirmation steps read `result.data.status === 'confirmed'` / `result.data.outputAmount`, but `SwapConfirmationStatus` is `{confirmed, failed?, blockNumber?, timestamp?, actualOutputAmount?}` — so a successful on-chain confirmation prints "Unexpected status: undefined" and exits 1. Dormant in herobids (same shape), migrated verbatim; only the human-gated `--execute` path is affected. Copy-never-author forbids fixing in traderton's copy. | Fix `status`/`outputAmount` reads in herobids' `scripts/ts/validate-*-launch.ts` (the source), re-copy to traderton, and do the real `--execute` live-venue verification with an operator's funded wallet. Also consider adding `scripts/tsconfig.json` typecheck coverage so tsx-invisible type errors surface. | Med | Low | Low |
